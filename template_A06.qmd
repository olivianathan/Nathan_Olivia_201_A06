...
for (player_name in names(player_info)) {
  ...
  points_per_game <- append(points_per_game, player$points / player$games_played)
  names(points_per_game)[length(points_per_game)] <- player_name
}
...
```{r}
points_per_game <- as.numeric(unlist(points_per_game))

rounded_points_per_game <- round(points_per_game, digits = 2)
...
avg_temps <- list()
for (month in 3:14) {
  avg_temps[[month]] <- mean(max_temps[[month]])
}
avg_temps
```
...
max_median <- which.max(median_temps)
year_max <- CO_temps$year[max_median]

min_median <- which.min(median_temps)
year_min <- CO_temps$year[min_median]
