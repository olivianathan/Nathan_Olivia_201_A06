...
[https://github.com/olivianathan/Nathan_Olivia_201_A06](https://github.com/olivianathan/Nathan_Olivia_201_A06)
...
```{r}
player_info[[2]][4] # With indexing
player_info$LJ$points # With names
```
...
```{r}
heights <- c(78, 81, 78, 86, 85) # List of heights
...
```{r}
points_per_game <- lapply(player_info, function(x) {x$points/x$games_played})
...
r_points_per_game <- sapply(points_per_game, round, digits = 2)
r_points_per_game
...
```{r}
mat_rand_rounded <- apply(mat_rand, MARGIN = 1:2, function(x){
  if (x<100) {
    round (x, -1)
  } else if (x<1000){
    round (x, -2)
...
```{r}
median_temps <- apply(CO_temps[, 3:14], 1, median)

max_median <- which.max(median_temps) #Finding max value
year_max <- CO_temps$year[max_median] #Finding max year

min_median <- which.min(median_temps) #Finding min value
year_min <- CO_temps$year[min_median] #Finding min year

paste0("The maximum median annual temperature of ", median_temps[max_median], " occured in ", year_max, " and the minimum median annual temperature of ", median_temps[min_median], " occured in ", year_min )
...
digit_sum <- function(n) {
  ...
  if (n < 10){ # Base case
    ...
    return(last_digit + digit_sum((n-last_digit)/10))
  }
}
digit_sum(935068)
```
